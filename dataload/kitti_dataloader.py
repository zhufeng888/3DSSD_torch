...
from utils.points_filter import get_point_filter, get_point_filter_in_image
...
class KittiDataset(torch_data.Dataset):
    ...
    def __init__(self, cfg, mode, split='training', img_list='trainval', is_training=True):
        """
        ...
        if not cfg.TEST.WITH_GT:
            base_dir += 'no_gt/'
        print('self. cfg.DATASET.KITTI.SAVE_NUMPY_PATH=', cfg.DATASET.KITTI.SAVE_NUMPY_PATH)
        ...
        if mode == 'loading':
            ...
            if self.is_training:
                self.data_augmentor = DataAugmentor(cfg, 'KITTI',cfg.num_workers)
    ...
    def load_samples(self, sample_idx):
        ...
        if self.is_training or self.cfg.TEST.WITH_GT:
            ...
            ry_cls_label, residual_angle = encode_angle2class_np(label_boxes_3d[:, -1],
                                                                 num_class=self.cfg.MODEL.ANGLE_CLS_NUM)
        else:
        ...
        if pts_num >= self.cfg.MODEL.POINTS_NUM_FOR_TRAINING:
            sampled_idx = np.random.choice(pts_idx, self.cfg.MODEL.POINTS_NUM_FOR_TRAINING, replace=False)
        else:
            ...
            sampled_idx_2 = np.random.choice(pts_idx, self.cfg.MODEL.POINTS_NUM_FOR_TRAINING - pts_num, replace=True)
    ...
    def preprocess_samples(self, indices):
        ...
        for sample_idx in indices:
            ...
            points = caproject_velo_to_rect(points)
    ...
    def evaluation(self, det_annos, gt_annos):
        import dataset.kitti_object_eval_python.eval as kitti_eval

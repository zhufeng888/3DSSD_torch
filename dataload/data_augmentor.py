...
from .mixup_sampler import MixupSampler


class DataAugmentor:
    def __init__(self, cfg, dataset, workers_num=1):
        """
        Data Augmentor,
        ...
        # flip augmentation
        self.aug_flip = self.cfg.TRAIN.AUGMENTATIONS.FLIP
        self.aug_flip_prob = 0.5

        # mixup augmentation
        self.mixup = self.cfg.TRAIN.AUGMENTATIONS.MIXUP.OPEN
        if self.mixup:
            self.mixup_class = self.cfg.TRAIN.AUGMENTATIONS.MIXUP.CLASS
            self.mixup_number = self.cfg.TRAIN.AUGMENTATIONS.MIXUP.NUMBER
            self.workers_num = workers_num
            self.mixup_sampler_list = []
            for i in range(self.workers_num):
                mixup_sampler = MixupSampler(self.cfg)
                self.mixup_sampler_list.append(mixup_sampler)

        # Single object augmentaiton
        self.aug_type = self.cfg.TRAIN.AUGMENTATIONS.PROB_TYPE
        self.single_aug_prob = self.cfg.TRAIN.AUGMENTATIONS.PROB 


    def kitti_forward(self, points, sem_labels, sem_dists, label_boxes_3d, label_classes, plane):
        """
        ...
        if self.mixup:
            mixup_result = self.kitti_mixup_sampling(points, sem_labels, sem_dists, label_boxes_3d, label_classes, plane)
        ...
        if choice[0] <= self.single_aug_prob[0]:
            ...
            label_boxes_3d, points = noise_per_object_v3_(self.cfg, label_boxes_3d, points, valid_mask=gt_boxes_mask, rotation_perturb=self.cfg.TRAIN.AUGMENTATIONS.SINGLE_AUG.ROTATION_PERTURB, center_noise_std=self.cfg.TRAIN.AUGMENTATIONS.SINGLE_AUG.CENTER_NOISE_STD, random_scale_range=self.cfg.TRAIN.AUGMENTATIONS.SINGLE_AUG.RANDOM_SCALE_RANGE, global_random_rot_range=[0., 0.], scale_3_dims=self.cfg.TRAIN.AUGMENTATIONS.SINGLE_AUG.SCALE_3_DIMS, sem_labels=sem_labels)
            label_boxes_3d = label_boxes_3d[gt_boxes_mask]
            label_classes = label_classes[gt_boxes_mask]

        if choice[1] <= self.single_aug_prob[1]:
            random_angle = (np.random.rand() * 2 - 1) * self.cfg.TRAIN.AUGMENTATIONS.RANDOM_ROTATION_RANGE
        ...
        if choice[2] <= self.single_aug_prob[2]:
            random_scale = (np.random.rand() * 2 - 1) * self.cfg.TRAIN.AUGMENTATIONS.RANDOM_SCALE_RANGE + 1
    ...
    def kitti_mixup_sampling(self, points, sem_labels, sem_dists, label_boxes_3d, label_classes, plane):
        mixup_sampler = self.mixup_sampler_list[0]
        # TODO: test multi-mixup_sampler

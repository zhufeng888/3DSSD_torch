...
from utils.anchor_decoder import decode_dist_anchor_free
from utils.anchor_encoder import (encode_angle2class_torch,
                                      encode_dist_anchor_free)
...
class EncoderDecoder:
    ...
    def encode(self, center_xyz, assigned_gt_boxes, batch_anchors_3d):
        ...
        encoded_angle_cls, encoded_angle_res = encode_angle2class_torch(gt_angle, self.cfg.MODEL.ANGLE_CLS_NUM)
    ...
    def decode(self, center_xyz, det_offset, det_angle_cls, det_angle_res, is_training, batch_anchors_3d):
        ...
        det_angle_cls = det_angle_cls.view(bs, points_num * cls_num, self.cfg.MODEL.ANGLE_CLS_NUM)
        det_angle_res = det_angle_res.view(bs, points_num * cls_num, self.cfg.MODEL.ANGLE_CLS_NUM)
        batch_anchors_3d = batch_anchors_3d.view(bs, points_num * cls_num, -1)

        if self.regression_method == 'Dist-Anchor-free':
            pred_anchors_3d = self.decoder(self.cfg, center_xyz, det_offset, det_angle_cls, det_angle_res, is_training)
        else:
            pred_anchors_3d = self.decoder(self.cfg,det_offset, det_angle_cls, det_angle_res, batch_anchors_3d, is_training)

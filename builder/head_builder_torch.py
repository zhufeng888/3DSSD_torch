...
from utils.head_util_torch import BoxRegressionHead
...
class HeadBuilder(nn.Module):
    def __init__(self, cfg, batch_size, anchor_num, head_idx, head_cfg, is_training):
        ...
        self.head_predictor = BoxRegressionHead(cfg, self.pred_cls_channel, self.pred_reg_base_num, self.pred_reg_channel_num,
                                                self.is_training, self.head_cfg.PREDICT_ATTRIBUTE_AND_VELOCITY,
                                                self.bn, pre_channel)

    def forward(self, xyz_list, feature_list, output_dict,bn_decay=None):
        xyz_input = []

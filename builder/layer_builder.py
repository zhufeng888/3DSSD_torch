...
from utils.layers_util import Pointnet_sa_module_msg, Vote_layer
from utils.layers_util2 import Pointnet_sa_module_msg as Pointnet_sa_module_msg2


class LayerBuilder(nn.Module):
    def __init__(self, cfg, layer_idx, is_training, layer_cfg, fps_idx_list=None, bn_decay=None):
        super().__init__()
        ...
        if self.layer_type == 'SA_Layer':
            if self.xyz_index[0] == 0:
                self.layer_module = Pointnet_sa_module_msg(cfg,
                ...
                self.layer_module = Pointnet_sa_module_msg2(cfg,
                   self.radius_list, self.nsample_list,
                   self.mlp_list, self.is_training, bn_decay, self.bn,
            ...
            self.layer_module = Vote_layer(cfg, self.mlp_list, self.bn, self.is_training, self.pre_channel)

    def forward(self, xyz_list, feature_list, fps_idx_list, output_dict, bn_decay=None):
        """
        Build layers
        """
        ...
        if self.layer_type == 'SA_Layer':
            new_xyz, new_points, new_fps_idx = self.layer_module(xyz_input[0], feature_input[0], former_fps_idx, vote_ctr, bn_decay=bn_decay)
            xyz_list.append(new_xyz)
            feature_list.append(new_points)
            fps_idx_list.append(new_fps_idx)

        elif self.layer_type == 'Vote_Layer':
            new_xyz, new_points, ctr_offsets = self.layer_module(xyz_input[0], feature_input[0], bn_decay=bn_decay)

...
def decode_class2angle(pred_cls, pred_res_norm, bin_size, bin_interval, bin_offset=0.):
    ...
    """
    pred_cls_onehot = torch.nn.functional.one_hot(pred_cls, num_classes=bin_size)
    pred_angle_res_norm = torch.sum(pred_cls_onehot.float() * pred_res_norm, dim=-1)
    pred_res = (pred_cls.float() + pred_angle_res_norm + bin_offset) * bin_interval
    return pred_res
...
def decode_dist_anchor_free(cfg, center_xyz, det_forced_6_distance, det_angle_cls, det_angle_res, is_training):
    """
    ...
    """
    bs, points_num, _ = center_xyz.shape

    det_angle_cls = torch.argmax(det_angle_cls, dim=-1)
    pred_angle = decode_class2angle(det_angle_cls, det_angle_res, bin_size=cfg.MODEL.ANGLE_CLS_NUM, bin_interval=2 * np.pi / cfg.MODEL.ANGLE_CLS_NUM)
    pred_angle = torch.unsqueeze(pred_angle, dim=-1)
    ...
    padding_zeros = torch.zeros_like(padding_half_height)
    padding_translate = torch.stack([padding_zeros, padding_half_height, padding_zeros], dim=-1)
    ctr_xyz += padding_translate
    lhw = torch.clamp(half_distance * 2., min=0.1)

    pred_anchors_3d = torch.cat([ctr_xyz, lhw, pred_angle], dim=-1)
    return pred_anchors_3d

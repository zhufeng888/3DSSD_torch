...
from utils.kitti_util import project_to_image
...
def project_to_bev_torch(anchors):
    """
    Projects an array of 3D anchors into bird's eye view
    ...
    # we project the anchors to the bev maps
    x, y, z, l, h, w = torch.split(anchors, 1, dim=-1)
    half_dim_x = l / 2.0
    half_dim_z = w / 2.0
    ...
    z_max = z + half_dim_z
    # thus now, we get the project anchor
    bev_anchors = torch.cat([x_min, z_min, x_max, z_max], dim=-1)

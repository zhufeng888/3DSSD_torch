...
def nn_distance(pc1, pc2, l1smooth=False, l1=False):
    """
    Calculating square distance between a and b
    pc1: [bs, ndataset, c]
    pc2: [bs, npoint, c]
    ...
    N = pc1.shape[1]
    M = pc2.shape[1]
    pc1_expand_tile = pc1.unsqueeze(2).repeat(1, 1, M, 1)  # [bs, ndataset, npoint, c]
    pc2_expand_tile = pc2.unsqueeze(1).repeat(1, N, 1, 1)  # [bs, ndataset, npoint, c]
...
def merge_head_prediction(start_idx, output_dict, keys):
    ...
    for key in keys:
        ...
        merged_output = torch.cat(cur_output, dim=1)
